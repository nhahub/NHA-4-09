import 'package:equatable/equatable.dart';
import 'package:flutter_bloc/flutter_bloc.dart';
import 'package:moodly/features/mood/data/models/mood_model.dart';
import 'package:moodly/features/mood/data/repos/mood_repo.dart';

part 'mood_state.dart';

class MoodCubit extends Cubit<MoodState> {
  final MoodRepo repo;
  final String userId;

  int? selectedIndex;

  MoodCubit({required this.repo, required this.userId})
    : super(MoodInitialState());

  Future<void> checkTodayMood() async {
    emit(MoodCheckingState());
    final exists = await repo.hasDailyMood(userId);
    if (exists) {
      emit(MoodSavedState());
    } else {
      emit(MoodEmptyState());
    }
  }

  void selectMood(int index) {
    selectedIndex = index;
    emit(MoodSelectedState(index: index));
  }

  Future<void> saveDailyMood(MoodModel model) async {
    emit(MoodSavingState());
    try {
      await repo.saveDailyMood(model, userId);
      emit(MoodSavedState());
    } catch (e) {
      emit(MoodErrorState(message: e.toString()));
    }
  }
}
